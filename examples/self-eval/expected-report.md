...
- **Intent**: Self-evaluate the Rubrix v0.1 Phase 1 scaffold (schema + plugin manifest).
- **State**: IntentDrafted
...
| state_enum_complete | 0.25 | 1 | cli/schemas/rubrix.schema.json state enum lists all 10 lifecycle states (IntentDrafted, RubricDrafted, RubricLocked, MatrixDrafted, MatrixLocked, PlanDrafted, PlanLocked, Scoring, Passed, Failed) in the documented order. |
| lock_invariants_encoded | 0.25 | 0.8 | cli/schemas/rubrix.schema.json conditional allOf branches encode the lock invariants for every state in its enum, not only the Locked/Scoring states. |
