...
```
요청 → intent 보정 → 평가 기준 작성 → 증거 매트릭스 → 실행 계획 → 작업 → 채점 → 통과/실패
         /rubrix:brief    /rubrix:rubric    /rubrix:matrix   /rubrix:plan          /rubrix:score
```

- intent.brief가 보정되기 전에는 `/rubrix:rubric` 호출이 차단됩니다 (`RUBRIX_SKIP_BRIEF=1`로 우회 가능).
...
| Skill | 하는 일 | 상태 전이 |
|---|---|---|
| `/rubrix:brief` | `intent.brief` 보정 (project_type, situation, ambition, axis_depth) | IntentDrafted (idempotent) |
...
| 명령 | 역할 | 주요 exit code |
|---|---|---|
| `validate <path>` | 스키마 검증 (v1.1+: brief 미calibrated → stderr warning) | 0 유효 · 1 무효 |
| `brief init/get <path>` | `intent.brief` 보정 또는 조회 (`--axis`로 effective depth) | 0 성공 · 2 잘못된 호출 · 3 IntentDrafted 외 거부 |
| `state get/set <path>` | 상태 조회·전이 | 0 성공 · 3 불법 전이 |
| `lock <key> <path>` | artifact 잠금 (무결성 검사 포함) | 0 성공 · 3 무결성 실패 |
| `gate <path> [--apply]` | 임계값·하한선 평가 (v1.1+: deep 축 effective floor 강화) | 0 통과 · 4 실패 |
...
| Hook | 동작 |
|---|---|
| PreToolUse | 세 lock 모두 true 전까지 Edit/Write 차단. v1.1+: brief 미calibrated 시 `/rubrix:rubric` 차단 (`RUBRIX_SKIP_BRIEF=1` 우회). stdin timeout 시 fail-closed |
| UserPromptExpansion | state+locks 컨텍스트 주입; plan 미잠금 시 `/rubrix:score` 차단. v1.1+: brief 미calibrated 시 `<rubrix-suggestion>` 힌트 |
| Stop | Failed 상태에서 종료 차단 |
| SessionStart / PostToolUse / PostToolBatch / SubagentStop | 상태 정보 전달; stdin timeout 시 fail-open |
