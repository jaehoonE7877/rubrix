...
주요 기준 문서는 [`PLUGIN-README.md`](PLUGIN-README.md) (사용자 가이드) + [`docs/lifecycle-state-machine.md`](docs/lifecycle-state-machine.md) (라이프사이클 상세). [`docs/extensible-plan.md`](docs/extensible-plan.md)는 v1.0 surface가 어떻게 도착했는지의 설계 기록 + v1.1+ 로드맵.

## 현재 상태 (v1.0.0)

`.claude-plugin/`, `skills/`, `agents/`, `hooks/`, `cli/`, `scripts/`, `examples/`, `registry/` 모든 표면이 구현 완료. `claude plugin validate .` 통과, `cli/tests/` 79 vitest pass, iteration-4 with_skill 96.9% (+43.1pp delta).
