...
```text
packages/rubrix-claude-plugin/
├── .claude-plugin/
│   └── plugin.json             # Claude Code 플러그인 메타데이터
├── skills/
│   ├── rubric/SKILL.md         # /rubric
│   ├── matrix/SKILL.md         # /matrix
│   ├── plan/SKILL.md           # /plan
│   └── score/SKILL.md          # /score
├── agents/
...
```json
{
  "SessionStart": "${CLAUDE_PLUGIN_ROOT}/hooks/scripts/session_start.sh",
  "UserPromptExpansion": "${CLAUDE_PLUGIN_ROOT}/hooks/scripts/user_prompt_expansion.sh",
  "PreToolUse": "${CLAUDE_PLUGIN_ROOT}/hooks/scripts/pre_tool_use.sh",
  "PostToolUse": "${CLAUDE_PLUGIN_ROOT}/hooks/scripts/post_tool_use.sh",
  "PostToolBatch": "${CLAUDE_PLUGIN_ROOT}/hooks/scripts/post_tool_batch.sh",
  "SubagentStop": "${CLAUDE_PLUGIN_ROOT}/hooks/scripts/subagent_stop.sh",
  "Stop": "${CLAUDE_PLUGIN_ROOT}/hooks/scripts/stop.sh"
}
```
...
이를 통해 hoyeon 수준의 orchestration에 근접하게 만듭니다.
...
Rubrix는 더 이상 “문서화 전용 runtime”이 아닙니다. 이 계획은 Rubrix를 Claude Code harness로 설계하고, v0부터 hooks를 강제함으로써 hoyeon과 유사한 확장성을 확보합니다.
...
이 계획을 따라 Rubrix는 hoyeon과 비슷한 수준의 기능과 확장성을 갖춘 Claude Code harness로 발전할 수 있습니다.
