...
describe("Stage 3 multi-trigger logic (Codex critical #2)", () => {
  ...
  it("trigger (a): stage2 self_reported_confidence < threshold AND stage1 ambiguous → fires", () => {
    ...
    runCascadeForTest(makeCriterion({ axis: "data" }), defaultPolicy({ stage3_threshold: 0.7, stage3_axes: [] }), deepBriefContract({ axis_depth: { security: "standard", data: "standard", correctness: "standard", ux: "standard", perf: "standard" } }), {
  ...
  it("trigger (b): criterion axis in stage3_axes AND axis_depth=deep → fires", () => {
    ...
    runCascadeForTest(makeCriterion({ axis: "security" }), defaultPolicy({ stage3_axes: ["security"], stage3_threshold: 0.5 }), deepBriefContract(), {
      mechanicalChecker: stubs.mechanicalChecker,
      semanticJudge: stubs.semanticJudge,
      consensusPanel: stubs.consensusPanel,
    });
  ...
  it("trigger (c): stage1 emits explicit conflict_signal → fires", () => {
    ...
    runCascadeForTest(makeCriterion({ axis: "data" }), defaultPolicy({ stage3_axes: [], stage3_threshold: 0.5 }), deepBriefContract({ axis_depth: { security: "standard", data: "standard", correctness: "standard", ux: "standard", perf: "standard" } }), {
      mechanicalChecker: stubs.mechanicalChecker,
      semanticJudge: stubs.semanticJudge,
      consensusPanel: stubs.consensusPanel,
    });
  ...
  it("NEGATIVE: low self_reported_confidence ALONE (no stage1 ambiguity, no other signal) does NOT trigger Stage 3", () => {
    ...
    const result = runCascadeForTest(makeCriterion({ axis: "data" }), defaultPolicy({ stage3_threshold: 0.7, stage3_axes: [] }), deepBriefContract({ axis_depth: { security: "standard", data: "standard", correctness: "standard", ux: "standard", perf: "standard" } }), {
      mechanicalChecker: stubs.mechanicalChecker,
      semanticJudge: stubs.semanticJudge,
      consensusPanel: stubs.consensusPanel,
    });
